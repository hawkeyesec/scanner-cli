...
module.exports = function Rc (options) {
  ...
  const defaultFileLimit = Number.MAX_SAFE_INTEGER
  ...
  let self = {
    ...
    errorExclude: {},
    logger: util.defaultValue(options.logger, () => require('./logger')()),
    failOn: 'low',
    modules: ['all'],
    all: false,
    staged: false,
    ...
    if (fs.existsSync(rcFile)) {
      ...
      const handlers = {
        ...
        excludeErrors: errors => {
          errors.forEach(self.withErrorExclude)
        }
      }
  ...
  addProperty('withErrorExclude', withErrorExclude)
