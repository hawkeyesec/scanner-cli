...
module.exports = function FileManager(options) {
  ...
  options = util.permittedArgs(options, ['target', 'all', 'logger', 'exclusions']);
  options.exclusions = util.defaultValue(options.exclusions, []);
  ...
  options.exclusions = options.exclusions.concat(globalExclusions);
  logger.log('Exclusion patterns:', options.exclusions.join(', '));
  options.exclusions = options.exclusions.map(e => {
    return new RegExp(e);
  });

  util.enforceArgs(options, ['target', 'exclusions']);
  const self = {
