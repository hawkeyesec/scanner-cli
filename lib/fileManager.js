...
module.exports = function FileManager(options) {
  ...
  self.fileWithNameExists = fileName => {
    let fileNames = Object.keys(files).map(filePath => {
       const pathsSubstrings = filePath.split('/');
       return pathsSubstrings[pathsSubstrings.length -1];
    });
    if (fileNames.indexOf(fileName) > -1) return true;
    return false;
  };

  self.getFilePath = fileName => {
    const filesPathWithName =  Object.keys(files).filter(file => file.endsWith(`${fileName}`));
    if(filesPathWithName.length < 1) {
      throw new Error(`File ${fileName} does not exist`);
    }
    return filesPathWithName[0].replace(fileName, '');
  };
