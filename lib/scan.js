...
module.exports = function Scan (rc = {}) {
  ...
  const exec = rc.exec || new Exec({ logger })
  ...
  self.start = async function (done) {
    ...
    const inactiveModules = enabledModules.filter(module => !module.handles({ logger, exec, fm }))
    inactiveModules.forEach(module => logger.log('Skipping module'.bold, module.name))

    const activeModules = enabledModules.filter(module => module.handles({ logger, exec, fm }))
    for (const module of activeModules) {
      try {
        logger.log('Running module'.bold, module.name)
        const results = new ModuleResults(module.key)
        allResults.push(results)
        await module.run({ logger, exec, fm, results })
      } catch (e) {
