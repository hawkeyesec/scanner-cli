...
module.exports = function Scan (rc) {
  ...
  const logger = util.defaultValue(rc.logger, () => new Logger())
  ...
  const trimOutput = results => {
    ...
    results.forEach(moduleResult => {
      Object.keys(moduleResult.results).forEach(key => {
        moduleResult.results[key] = moduleResult.results[key].filter((result) => {
          if (rc.errorExclude.hasOwnProperty(constructExclusionCode(result))) {
            exclusionCount++
            return false
          }
          return true
  ...
  const constructExclusionCode = function (result) {
    return `${result.code}:${result.offender}`
  }
  self.start = function (done) {
    ...
    if (whichModules[0] === 'all') {
      modulesToRun = modules.filter(m => m.enabled)
    } else {
