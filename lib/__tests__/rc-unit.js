...
describe('RC', () => {
  ...
  describe('isExcluded', () => {
    it('should NOT exclude if no exclude pattern is given', () => {
      expect(new Rc().isExcluded('some-code')).to.be.false
    })

    it('should be excluded if the code matches the given pattern', () => {
      const rc = new Rc().withExclude('excluded-code')
      expect(rc.isExcluded('excluded-code')).to.be.true
    })

    it('should be excluded if at least one of the patterns matches', () => {
      const rc = new Rc().withExclude('some-pattern').withExclude('excluded-code')
      expect(rc.isExcluded('excluded-code')).to.be.true
    })

    it('should exclude project root `node_modules` by default', () => {
      expect(new Rc().isExcluded('node_modules/commander/index.js')).to.be.true
    })

    it('should exclude project root `.git` contents by default', () => {
      expect(new Rc().isExcluded('.git/hooks/pre-commit')).to.be.true
    })

    it('should exclude project root `package-lock.json` by default', () => {
      expect(new Rc().isExcluded('package-lock.json')).to.be.true
    })
  })
})
