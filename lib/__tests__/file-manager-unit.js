...
describe('File Manager', () => {
  ...
  describe('fileLimit', () => {
    ...
    it('expect pick a subset of files based on fileLimit', (done) => {
      fm = new FileManager({
        target: tempDir,
        fileLimit: 10
      })
    ...
    it('expect scan all files when fileLimit is not set', (done) => {
      fm = new FileManager({
        target: tempDir
      })
      const result = fm.all()
  ...
  it('should const me get a files contents', done => {
    fm.readFile('test/file3', (err, contents) => {
      ...
      expect(contents).to.equal('this is file 3')
      done()
    })
  })
