...
describe('ScanResults', () => {
  describe('allWithLevelAtLeast', () => {
    it('should NOT return the result that has "low" level, but "critical" level is requested', () => {
      let scanResults = new ScanResults([{ level: 'low' }])
    ...
    it('should return the result that has "high" level and "medium" level is requested', () => {
      let scanResults = new ScanResults([{ level: 'high' }])
    ...
    it('should return results with level above or equal the requested one', () => {
      ...
      for (const resultLevel in levels) {
        let scanResults = new ScanResults([{ level: resultLevel }])
    ...
    it('should filter out results with level below the requested one', () => {
      ...
      for (const lowerLevel in levels) {
        let scanResults = new ScanResults([{ level: lowerLevel }])
  ...
  describe('fromModuleResultsList', () => {
    it('should assign correct module name to results', () => {
      let moduleResults = new ModuleResults('test-mod')
      moduleResults.critical({})

      let scanResults = ScanResults.fromModuleResultsList([moduleResults])
      let displayResult = scanResults.allWithLevelAtLeast('low')[0]
    ...
    it('should assign correct level to results', () => {
      ...
      let scanResults = ScanResults.fromModuleResultsList([moduleResults])
      let displayResult = scanResults.allWithLevelAtLeast('critical')[0]

      expect(displayResult.level).to.be.equal('critical')
    })

    it('should join results of two modules in a single array', () => {
      let moduleResults1 = new ModuleResults('test-mod1')
      moduleResults1.critical({})
      let moduleResults2 = new ModuleResults('test-mod2')
      moduleResults2.high({})

      let scanResults = ScanResults.fromModuleResultsList([moduleResults1, moduleResults2])
      let displayResult1 = scanResults.allWithLevelAtLeast('low')[0]
      let displayResult2 = scanResults.allWithLevelAtLeast('low')[1]
