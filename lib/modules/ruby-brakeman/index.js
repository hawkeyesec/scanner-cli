...
module.exports = function Brakeman(options) {
  ...
  const isRailsApp = (subdirectoryPath) => {
    return fileManager.readFileSync(`${subdirectoryPath}Gemfile`).indexOf('rails') >= 0;
  };

  const containsAppFolder = (subdirectoryPath) => {
    return fileManager.exists(`${subdirectoryPath}app`);
 };

  const isRubyProject = () => {
    return fileManager.fileWithNameExists('Gemfile');
  };

  self.handles = function(manager) {
    util.enforceType(manager, Object);
    fileManager = manager;
    let subdirectoryPath = fileManager.target;

    if(isRubyProject()) {
      subdirectoryPath = fileManager.getFilePath('Gemfile');
    }
    const shouldHandle =  isRubyProject() && isRailsApp(subdirectoryPath);
    ...
    if(shouldHandle && !containsAppFolder(subdirectoryPath)) {
  ...
  self.run = function(results, done) {
    ...
    const projectPath = fileManager.getFilePath('Gemfile');

    options.exec.command(brakemanCommand, {cwd: projectPath}, () => {
