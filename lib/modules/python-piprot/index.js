...
class Vulnerability {
  ...
  get description () {
    if (this.severity === MEDIUM_SEVERITY) { return 'Module is one or more minor versions out of date' }
    if (this.severity === HIGH_SEVERITY) { return 'Module is one or more major versions out of date' }
  ...
  get severity () {
    if (!this._versionIsValid(this.currentVersion) || !this._versionIsValid(this.latestVersion)) { return LOW_SEVERITY }

    if (semver.minor(this.currentVersion) < semver.minor(this.latestVersion)) { return MEDIUM_SEVERITY }

    if (semver.major(this.currentVersion) < semver.major(this.latestVersion)) { return HIGH_SEVERITY }
    return LOW_SEVERITY
  }
}
...
class Logger {
  ...
  _getCode () {
    if (this.vulnerability.severity === HIGH_SEVERITY) { return 1 }
    if (this.vulnerability.severity === MEDIUM_SEVERITY) { return 2 }
    return 3
  }
...
module.exports = function PythonOutdatedDep (options) {
  ...
  self.run = (results, done) => {
    options.exec.command('piprot -o', {
      ...
      if (vulnerabilities.length === 0) { return done() }
