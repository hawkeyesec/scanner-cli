...
module.exports = {
  ...
  run: async fm => {
    ...
    const command = `dependency-check --noupdate --project Testing --format JSON --out . -s ${jarFiles}`
    const data = await exec.command(command)

    if (!fm.exists('dependency-check-report.json')) {
      throw new Error(`There was an error while executing Owasp Dependency Check and the report was not created: ${JSON.stringify(data.stderr)}`)
    }
