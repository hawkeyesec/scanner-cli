...
describe('Ruby Bundler Module', () => {
  ...
  beforeEach(() => {
    ...
    exec.command.withArgs('bundle-audit update', { cwd: target, shell: '/bin/bash' }).resolves()
    exec.command.withArgs('bundle-audit', { cwd: target, shell: '/bin/bash' }).resolves({ stdout: sample })
