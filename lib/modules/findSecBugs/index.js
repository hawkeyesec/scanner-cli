...
module.exports = function FindSecBugs(options) {
  ...
  self.run = function(results, done) {
    const findSecBugsCommand = `findsecbugs -nested:false -progress -effort:max -exitcode -xml:withMessages -output ${fileManager.target}/findSecBugsReport.xml /target`;
    const findSecBugsLocation = options.exec.commandSync(`which findsecbugs`).stdout;

    options.exec.command(findSecBugsCommand, {cwd: findSecBugsLocation }, (err, data) => {
